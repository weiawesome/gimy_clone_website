            ...
            {firstFilmInformation!==undefined &&(
                <div className={"relative mt-2.5 flex justify-between truncate"}>
                    ...
                    <div className={"truncate text-sm text-normal-color items-start w-1/2 pl-3"}>
                        <p className={"truncate text-md font-bold"}>{firstFilmInformation.title}</p>
                        <p className={"mt-5 truncate"}>{firstFilmInformation.actors.join(",")}</p>
                        <p className={"mt-1 truncate"}>{firstFilmInformation.state}</p>
                    </div>
                    ...
                    <div className={"mt-2.5"} key={index}>
                        <div className={"flex flex-row justify-between items-center"}>
                            <Link href={"/resource/"+item.id} className={"flex flex-1 items-center text-normal-color hover:text-primary-color"}>
                                ...
                                <p className={"ml-3 text-sm truncate"}>{item.title}</p>
                            </Link>
                        </div>
                    </div>
                )
